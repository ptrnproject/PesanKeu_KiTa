...
function hitungOtomatisHargaTeksMentah(namaMenuTeks, kustomisasiTeks) {
    ...
    let totalHargaSatuan = hargaDasar;
    if (kustomisasiTeks) {
        const teksKustom = kustomisasiTeks.toLowerCase();
        
        // Aturan Lumpia Extra Telor (+4.000)
        if (teksKustom.includes("telor")) {
            totalHargaSatuan += 4000;
        }
        // Aturan Banana Extra Double Keju (+2.000)
        if (teksKustom.includes("double keju") || teksKustom.includes("extra double")) {
            totalHargaSatuan += 2000;
        }
    }
...
function renderOrdersTable() {
    ...
    Object.keys(localOrdersCache).forEach(key => {
        ...
        if (!order.tanggal || order.status === "selesai") return;

        // AMBIL DATA MENTAH DARI DATABASE
        let rawData = order.menu_order || "";
        
        // TAMPILKAN LANGSUNG DENGAN STYLE TAMBAHAN
        // Kita menggunakan style agar teks terlihat rapi dan kita paksa muncul di kolom Rincian Menu
        const rincianMenuHTML = `
            <div style="white-space: pre-line; font-size: 13px; color: #334155;">
                ${rawData}
            </div>
            <div style="margin-top: 5px; padding-top: 5px; border-top: 1px dashed #94a3b8; font-weight: bold; color: #0f766e;">
                Total Tagihan: ${order.total_bayar ? 'Rp ' + Number(order.total_bayar).toLocaleString('id-ID') : '-'}
            </div>
        `;
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge" style="background-color: ${order.status === 'PENDING' ? '#f59e0b' : '#3b82f6'}; color: white; padding: 4px 8px; border-radius: 4px;">${order.status.toUpperCase()}</span></td>
            <td>
